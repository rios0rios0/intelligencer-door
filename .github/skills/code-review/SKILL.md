---
name: code-review
description: "Review pull requests and diffs in intelligencer-door — the preserved Delphi 7 remote administration tool, kept as a historical security archive — against the rios0rios0/guide standards, with the strict framing that this repository documents a discontinued 2013 tool and is not to be extended. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `intelligencer-door`

`intelligencer-door` is a client/server remote administration tool with a connection list, file manager, key logger, screen logger, diagnostics, an icon changer, and a server builder that produces a configured payload. It is a **preserved historical archive**, discontinued in 2013, kept for study.
...
## What matters most in `intelligencer-door`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

- **This repository is closed for functional development.** It documents a discontinued tool. Acceptable changes are documentation, metadata, changelog, and CI. A pull request that adds a capability — new remote commands, persistence, evasion, propagation, a broader payload builder, or anything that makes deployment easier — is out of scope: say so plainly, decline the review, and do not suggest an alternative implementation.
- **The worm options and the server builder are the sharpest edge.** `UCS.pas` generates a configured server executable; `IconChanger.pas` rewrites an EXE's icon. Treat any change touching either as functional development under the rule above.
- **Embedded resources are opaque binaries** — `Resources/Server.res` carries the server template and `Icons.res` the selectable icons. A change that replaces one cannot be reviewed; require its provenance, and question why it is needed at all in an archive.
- **`Address.xlsx` and any list-like file must contain no real hosts or personal data.** A real address, credential, or victim record here is a **Critical** finding.
- **Documentation must keep the archival framing.** README and `CONTRIBUTING.md` explain what the tool was and when it stopped; a change that reads as operating instructions is a finding.
- **Socket input is untrusted by construction.** If a change does touch the protocol code, every length and index from the wire must be bounds-checked — but see the first rule: that change probably should not be happening.
- **Treat the code as an archive**: `T`-prefixed types, component-prefixed names, Delphi auto-generated event handlers, Portuguese UI strings. No modernisation.
- **`Clear.bat` defines build output**; none of it belongs in a commit.

### Commands a reviewer should be able to quote

```bash
# open ID.dpr in Borland Delphi 7 and compile (Ctrl+F9) — study only
Clear.bat
```
...
There is no test suite and none should be added — that would mean building a harness for the tool. Verification for a documentation or metadata change is reading it; a functional change is out of scope for this repository.
...
- The multi-form VCL structure and the `.dfm` designer output.
